    ...
    return (
        <div className="h-full w-48 flex flex-col items-center">
            {departureCityDetails.recievedInfo && (
                <Card className="relative w-48 h-[80%]  hover:cursor-grab shadow-lg rounded-xl flex flex-col justify-between">
                    {/* Image section */}
...
const CitySelection2 = () => {
    ...
    return (
        <div className='mt-[-50px]'>
            ...
            <div ref={sortableRef} className='flex flex-row space-x-4 overflow-x-auto' style={{width:"54rem"}}>
                {departureCityDetails.map((city) => (
                    <div key={city.id} className={`card ${city.locked ? "locked" : ""} p-0 `}>
                        <DraggableCityCard city={city} />
                        <Button label="Add Nights and SightSeeings" className='mt-[-25px] mb-2 p-1 h-10 text-xs font-bold bg-indigo-900 text-white pt-1 pb-1 pl-2 pr-2 rounded-xl ' onClick={showDialog} />
                        <NightsDialog
